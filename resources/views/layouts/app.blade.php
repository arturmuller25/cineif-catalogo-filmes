    ...
    <header class="sticky top-0 z-40 border-b border-zinc-800/80 bg-zinc-950/80 backdrop-blur">
        <nav class="mx-auto flex max-w-6xl flex-wrap items-center gap-x-4 gap-y-2 px-4 py-3">
            <a href="{{ route('galeria.index') }}" class="flex items-center gap-2 text-lg font-extrabold tracking-tight">
                <svg class="h-6 w-6 text-amber-400" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                ...
                <span>Cine<span class="text-amber-400">IF</span></span>
            </a>
            ...
            <div class="ml-auto flex flex-wrap items-center gap-1 text-sm">
                ...
                @auth
                    ...
                    <a href="{{ route('admin.filmes.index') }}"
                       class="rounded-lg px-3 py-2 text-zinc-300 transition hover:bg-zinc-800 hover:text-white {{ request()->routeIs('admin.filmes.index') ? 'text-white' : '' }}">
                        Gerenciar
                    </a>
                    <a href="{{ route('admin.filmes.create') }}"
                       class="rounded-lg bg-amber-400 px-3 py-2 font-semibold text-zinc-900 transition hover:bg-amber-300">
                        + Novo filme
                    </a>
                    <span class="mx-1 hidden text-zinc-600 sm:inline">|</span>
                    <span class="hidden text-xs text-zinc-400 sm:inline">{{ auth()->user()->name }}</span>
